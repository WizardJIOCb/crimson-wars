...
async function beginBattleHubPlayerSwapFx(nextHeroId = '') {
  ...
  preloadBattleHubHeroSkillImages(nextHero, game.playerAuth?.progressionCatalog || null);
...
function renderBattleHubHeroSkills(heroId, progression = null, catalog = null, loggedIn = false) {
  ...
  preloadBattleHubHeroSkillImages(heroId, catalog);
...
function renderBattleHubHeroSkillIcon(skill, fallbackIcon, extraClass = '') {
  ...
  if (imagePath) {
    return `<span class="${escapeHtml(className)}"><img src="${escapeHtml(imagePath)}" alt="" loading="eager" decoding="async" fetchpriority="high"></span>`;
  }
  return `<span class="${escapeHtml(className)}">${escapeHtml(fallbackIcon)}</span>`;
}

function preloadBattleHubHeroSkillImage(path) {
  const imagePath = String(path || '').trim();
  if (!imagePath || imagePath.startsWith('data:') || battleHubHeroSkillImagePreload.has(imagePath)) return;
  const img = new Image();
  img.decoding = 'async';
  img.src = imagePath;
  battleHubHeroSkillImagePreload.set(imagePath, img);
  img.decode?.().catch(() => {});
}

function preloadBattleHubHeroSkillImages(heroId, catalog = null) {
  const hero = getBattleHubHeroCatalogEntry(heroId, catalog);
  const skills = Array.isArray(hero?.uniqueSkills) ? hero.uniqueSkills : [];
  for (const skill of skills) {
    preloadBattleHubHeroSkillImage(getBattleHubHeroSkillIconPath(skill));
  }
}
...
function renderBattleHubHeroSkillModalArt(skill, fallbackIcon, rarity, color) {
  ...
  if (imagePath) {
    return `<div class="${escapeHtml(className)}" style="${escapeHtml(style)}"><img src="${escapeHtml(imagePath)}" alt="" loading="eager" decoding="async" fetchpriority="high"></div>`;
  }
...
const TRIPO_MAP_PROP_VERSION = '20260605dir12';
function tripoMapPropSrc(name) {
  return `${TRIPO_MAP_PROP_ROOT}/${name}.png?v=${TRIPO_MAP_PROP_VERSION}`;
}
const MAP_PROP_DIRECTIONAL_SPRITES = Object.freeze(Object.fromEntries([
  ...
  src: `${TRIPO_MAP_PROP_ROOT}/${key}_directions.png?v=${TRIPO_MAP_PROP_VERSION}`,
  frames: 12,
  columns: 4,
  angleOffset: 0,
  direction: 1,
...
const sprites = {
  ...
  mapProps: {
    ...
    bus_yellow: loadImage(tripoMapPropSrc('bus_yellow')),
    abandoned_bus: loadImage(tripoMapPropSrc('abandoned_bus')),
    ambulance: loadImage(tripoMapPropSrc('ambulance')),
    military_ambulance: loadImage(tripoMapPropSrc('military_ambulance')),
    barrier: loadImage(tripoMapPropSrc('barrier')),
    shack: loadImage('/assets/map-props/shack.png'),
    mall_block: loadImage(tripoMapPropSrc('mall_block')),
    clinic_block: loadImage(tripoMapPropSrc('clinic_block')),
    industrial_tank: loadImage(tripoMapPropSrc('industrial_tank')),
    reactor_block: loadImage(tripoMapPropSrc('reactor_block')),
    shopping_cart_barricade: loadImage(tripoMapPropSrc('shopping_cart_barricade')),
    build_1: loadImage(tripoMapPropSrc('build_1')),
    build_2: loadImage(tripoMapPropSrc('build_2')),
    build_3: loadImage(tripoMapPropSrc('build_3')),
    build_4: loadImage(tripoMapPropSrc('build_4')),
    build_5: loadImage(tripoMapPropSrc('build_5')),
    build_6: loadImage(tripoMapPropSrc('build_6')),
    build_7: loadImage(tripoMapPropSrc('build_7')),
    abandoned_building: loadImage(tripoMapPropSrc('abandoned_building')),
    abandoned_building_1: loadImage(tripoMapPropSrc('abandoned_building_1')),
    abandoned_building_2: loadImage(tripoMapPropSrc('abandoned_building_2')),
    abandoned_gas_station: loadImage(tripoMapPropSrc('abandoned_gas_station')),
    cyberpunk_storefront: loadImage(tripoMapPropSrc('cyberpunk_storefront')),
    futuristic_barrier: loadImage(tripoMapPropSrc('futuristic_barrier')),
    futuristic_vending_machine: loadImage(tripoMapPropSrc('futuristic_vending_machine')),
    glowing_twisted_tree: loadImage(tripoMapPropSrc('glowing_twisted_tree')),
    gnarled_tree: loadImage(tripoMapPropSrc('gnarled_tree')),
    haunted_house: loadImage(tripoMapPropSrc('haunted_house')),
    industrial_facility: loadImage(tripoMapPropSrc('industrial_facility')),
    industrial_factory: loadImage(tripoMapPropSrc('industrial_factory')),
    ruined_sci_fi_facility: loadImage(tripoMapPropSrc('ruined_sci_fi_facility')),
    ruined_storefront: loadImage(tripoMapPropSrc('ruined_storefront')),
    rusty_gas_station: loadImage(tripoMapPropSrc('rusty_gas_station')),
    rusty_shopping_cart: loadImage(tripoMapPropSrc('rusty_shopping_cart')),
    steampunk_reactor: loadImage(tripoMapPropSrc('steampunk_reactor')),
    toxic_waste_barrels: loadImage(tripoMapPropSrc('toxic_waste_barrels')),
    futuristic_police_vehicle: loadImage(tripoMapPropSrc('futuristic_police_vehicle')),
    post_apocalyptic_car: loadImage(tripoMapPropSrc('post_apocalyptic_car')),
    wrecked_police_car: loadImage(tripoMapPropSrc('wrecked_police_car')),
  },
  mapPropDirections: MAP_PROP_DIRECTIONAL_SPRITES,
  backgrounds: {
