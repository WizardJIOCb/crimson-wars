...
Документация по интеграционному запуску забега и callback-у с наградой:

- [docs/vamipre.integration.md](docs/vamipre.integration.md)
...
## ????????? (?????)
- ??? ????????? ????? ? ???????: UTF-8 (??? ????? ?????????).
- ?????? ???????????? ?????????: ?????? ????? PowerShell `Set-Content`/`Out-File` ??? `-Encoding UTF8`.
